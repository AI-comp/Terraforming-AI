...
public class Tower {
  ...
  private boolean buildTower(final Game game) {
    for (Point enemyInitialPoint : game.field
        .getPointsWithEnemyInitial(game.myId)) {
      List<Point> pointsToBuild = game.field.getPointsInRadius(
          enemyInitialPoint, DistanceBetweenTowerAndEnemyInitial, false);
      for (Point point : pointsToBuild) {
        if (game.field.tiles.containsKey(point)) {
          Tile tile = game.field.tiles.get(point);
          if (tile.canBuildInstallation(Installation.Tower, game.field,
              game.myId)) {
  ...
  private boolean buildBridge(final Game game) {
    for (Point point : game.field.getPointsWithRobots(game.myId)) {
      Tile tile = game.field.tiles.get(point);
      if (tile.isHole
          && tile.canBuildInstallation(Installation.Bridge, game.field,
              game.myId)) {
  ...
  private void setNumRequiredRobotsForInstallation(final Game game,
      Map<Point, Integer> numRequiredRobots,
    ...
    for (Point pointToBuildInstallation : pointsToBuildInstallation) {
      Utility.AddIntToMap(numRequiredRobots, pointToBuildInstallation,
          installation.robotCost);
      for (Point neighborPoint : game.field.getPointsInRadius(
          pointToBuildInstallation, 1, false)) {
        Tile neighborTile = game.field.tiles.get(neighborPoint);
        if (!neighborTile.isHole && neighborTile.installation.equals(Installation.None)) {
          Utility.AddIntToMap(numRequiredRobots, neighborPoint,
              NumRobotsToDefendConstruction);
        }
      }
    }
  }

  private void setNumRequiredRobotsForBridge(final Game game,
      Map<Point, Integer> numRequiredRobots) {
    ...
    setNumRequiredRobotsForInstallation(game, numRequiredRobots,
        pointsToBuildBridge, Installation.Bridge);
  }

  private void setNumRequiredRobotsForTower(final Game game,
      Map<Point, Integer> numRequiredRobots) {
    List<Point> pointsToBuildTower = new ArrayList<Point>();
    for (final Point enemyInitialPoint : game.field
        .getPointsWithEnemyInitial(game.myId)) {
      List<Point> candidatePoints = game.field.getPointsInRadius(
          enemyInitialPoint, DistanceBetweenTowerAndEnemyInitial, false);
      Collections.sort(candidatePoints, new Comparator<Point>() {
        private int getScore(Point point) {
          ...
          score -= game.field.getNumNeighborTilesWithInstallation(center,
      ...
      for (Point candidatePoint : candidatePoints) {
        ...
        if (!candidateTile.isHole
            && candidateTile.installation.equals(Installation.None)
            && game.field.getNumNeighborTilesWithInstallation(candidatePoint,
                Installation.None, 1) >= Installation.Tower.materialCost) {
    ...
    setNumRequiredRobotsForInstallation(game, numRequiredRobots,
        pointsToBuildTower, Installation.Tower);
  }

  private int getNumRobotsToMove(Point point, Tile tile,
      Map<Point, Integer> numRequiredRobots) {
    int numRobotsToMove;
  ...
  void moveRobots(final Game game, Map<Point, Integer> numRequiredRobots,
      Set<Point> pointsWithMovedRobots) {
    for (Point currentPoint : game.field.getPointsWithRobots(game.myId)) {
      ...
      int numRobotsToMove = getNumRobotsToMove(currentPoint, currentTile,
          numRequiredRobots);
      ...
      for (Entry<Point, Integer> destination : numRequiredRobots.entrySet()) {
        ...
        List<Point> shortestPath = algorithm.getShortestPath(currentPoint,
            destinationPoint, false, game.field, game.myId);
        if (shortestPath.isEmpty()) {
          shortestPath = algorithm.getShortestPath(currentPoint,
              destinationPoint, true, game.field, game.myId);
        }
      ...
      if (shortestPathToDestinations.size() > 0) {
        Collections.sort(shortestPathToDestinations,
            new Comparator<List<Point>>() {
              @Override
        ...
        move(
            currentPoint,
            currentPoint.getDirection(shortestPathToDestinations.get(0).get(0)),
            numRobotsToMove);
  ...
  void moveRemainingRobots(final Game game,
      Map<Point, Integer> numRequiredRobots, Set<Point> pointsWithMovedRobots) {
    for (Point currentPoint : game.field.getPointsWithRobots(game.myId)) {
      if (!pointsWithMovedRobots.contains(currentPoint)) {
        List<Point> candidateAlternatePoints = game.field.getPointsInRadius(
            currentPoint, 1, false);
        List<Point> alternatePoints = new ArrayList<Point>();
        for (Point candidateAlternatePoint : candidateAlternatePoints) {
          Tile candidateAlternateTile = game.field.tiles
              .get(candidateAlternatePoint);
          if (!candidateAlternateTile.isHole
              && candidateAlternateTile.canMoveInto(game.myId)) {
        ...
        if (numRobotsToMove > 0 && candidateAlternatePoints.size() > 0) {
          Point alternatePoint = candidateAlternatePoints.get(random
              .nextInt(candidateAlternatePoints.size()));
          assert (numRobotsToMove <= currentTile.robot);
          move(currentPoint, currentPoint.getDirection(alternatePoint),
              numRobotsToMove);
          pointsWithMovedRobots.add(currentPoint);
        }
      }
