...
public class Factory {
  ...
  private boolean buildFactory(final Game game) {
    for (Entry<Point, Tile> pointAndTile : game.field.tiles.entrySet()) {
      ...
      if (tile.canBuildInstallation(Installation.Robotmaker, game.field,
          game.myId)) {
  ...
  private void setNumRequiredRobotsForFactory(final Game game,
    ...
    for (Point candidatePoint : candidatePoints) {
      ...
      if (!candidateTile.isHole
          ...
          && game.field.getNumNeighborTilesWithInstallation(candidatePoint,
              null, 1) >= Installation.Robotmaker.materialCost) {
    ...
    setNumRequiredRobotsForInstallation(game, numRequiredRobots,
        pointsToBuildFactory, Installation.Robotmaker);
  }

  private void setNumRequiredRobotsForInvation(final Game game,
      Map<Point, Integer> numRequiredRobots) {
    ...
    Collections.sort(candidatePoints, new Comparator<Point>() {
      ...
      public int compare(Point left, Point right) {
        int leftDistance = game.field.getDistance(left,
            game.field.getPointWithMyInitial(game.myId));
        int rightDistance = game.field.getDistance(right,
            game.field.getPointWithMyInitial(game.myId));
    ...
    for (int i = 0; i < Math.min(NumPointsToInvadeAtOnce,
        candidatePoints.size()); i++) {
